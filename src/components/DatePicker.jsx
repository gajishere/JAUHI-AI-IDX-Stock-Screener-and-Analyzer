...
export function DatePicker({ value, onChange, min, max, placeholder = 'Select date', inline = false }) {
                    ...
                    return (
                      <td key={index} className={`relative ${inline ? 'h-12' : 'h-10'}`}>
                        <button
                          ...
                          className={`flex h-full w-full items-center justify-center rounded-md font-medium transition-[transform,opacity] duration-200 ${
                          ...
                          aria-label={`${day.isSelected ? 'Selected, ' : ''}${
                            day.isToday ? 'Today, ' : ''
                          }${day.isDisabled ? 'Unavailable' : ''} ${day.day}`}
                        >
                          {day.day}
                        </button>
                      </td>
  ...
  return (
    <div className="relative">
      ...
      {isOpen && (
        <div
          ref={popupRef}
          className="absolute z-dropdown mt-2 w-56 rounded-md border border-line bg-paper shadow-lg shadow-ink/5 w-full max-w-xs"
        >
          {calendarBody}
        </div>
      )}
